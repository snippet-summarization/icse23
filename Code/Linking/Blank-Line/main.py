...
def main():
	...
	data['blankBaselineOutput'] = blankBaselineOutput
	data['blankBaselineOutputFlattened'] = blankBaselineOutputFlattened
	data['blankBaselineSpan'] = blankBaselineSpan
	data = data.rename(columns={"modelOutput": "oracle"})

	#flattening the oracle

	oracleItems = list(data['oracle'])
	flattenedOracle = []
	for item in oracleItems:
		preparedTarget = ""
		for line in item.splitlines():
			preparedTarget += (line.strip() + ' <nl> ')

		flattenedOracle.append(preparedTarget)

	data['oracleFlattened'] = flattenedOracle
